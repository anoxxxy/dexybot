...
if (orderType === 'both' || orderType === 'buy') {
  ...
  var rbuyOrdersTotal = 0;
  var tmp = ych.data.buys[marketPair];

  if (tmp === undefined) return;
  var tmpLength = tmp.length;

  // 1. Calculate buyOrdersTotal
  for (var i = 0; i < tmp.length; i++) {
    var reverseI = tmpLength - i - 1;
    buyOrdersTotal += tmp[i].amounta / 1e8;
    rbuyOrdersTotal -= tmp[reverseI].amounta / 1e8;
  }

  $('[data-market="buy_orders_total"]').text(buyOrdersTotal.toFixed(8));

  // 2. Batch DOM updates using a document fragment
  var fragment = document.createDocumentFragment();

  // Loop through the order data and create the HTML elements
  for (var i = 0; i < tmp.length; i++) {
    var order = tmp[i];
    var entryContainer = document.createElement('div');
    entryContainer.className = 'entryContainer';

    var marketSizeBar = document.createElement('div');
    marketSizeBar.className = 'marketSizeBar';
    marketSizeBar.style.width = '0%';
    entryContainer.appendChild(marketSizeBar);

    var leftLabelCol = document.createElement('div');
    leftLabelCol.className = 'leftLabelCol';
    var leftLabel = document.createElement('span');
    leftLabel.className = 'entryText';
    leftLabel.textContent = (order.amounta / 1e8).toFixed(4);
    leftLabelCol.appendChild(leftLabel);
    entryContainer.appendChild(leftLabelCol);

    var centerLabelCol = document.createElement('div');
    centerLabelCol.className = 'centerLabelCol';
    var centerLabel = document.createElement('span');
    centerLabel.className = 'entryBuyText text-lime';
    centerLabel.setAttribute('data-book-row-price', order.price);
    centerLabel.setAttribute('data-book-row-amounta', order.amounta);
    centerLabel.setAttribute('data-book-row-amountb', order.amountb);
    centerLabel.textContent = (order.price / 1e8).toFixed(8);
    centerLabelCol.appendChild(centerLabel);
    entryContainer.appendChild(centerLabelCol);

    var rightLabelCol = document.createElement('div');
    rightLabelCol.className = 'rightLabelCol';
    var rightLabel = document.createElement('span');
    rightLabel.className = 'entryText';
    //rightLabel.textContent = rbuyOrdersTotal.toFixed(4);
    rightLabelCol.appendChild(rightLabel);
    entryContainer.appendChild(rightLabelCol);

    fragment.appendChild(entryContainer);

    // Calculate and update the market size bar width
    var leftPercent = Math.ceil((order.amounta / buyOrdersTotal) * 100);
    marketSizeBar.style.width = leftPercent + '%';
  }

  var buyEntryContainer = $('#xyOrderBookTable .entriesContainer[data-orders="buy"]');
  buyEntryContainer.html(fragment);

  // 3. Set background color relative to total in percent
  // 4. Update the market size bar width in xyOrderBook
  var buyTotal = 0;
  buyEntryContainer.find('.entryContainer').each(function(i, el) {
    var reverseI = tmpLength - i - 1;
    buyTotal += tmp[i].amounta / 1e8;



    var leftPercent = Math.ceil(parseFloat(buyTotal / buyOrdersTotal * 100));
...
if (orderType === 'both' || orderType === 'sell') {
  ...
  // Loop through the order data and update or add HTML elements
  var sellEntryContainer = $('#xyOrderBookTable .entriesContainer[data-orders="sell"]');
  sellEntryContainer.find('.entryContainer').each(function() {
    var orderPrice = parseFloat($(this).find('.entrySellText').attr('data-book-row-price'));

    var existingRow = tmp.find(function(order) {
      return order.price === orderPrice;
    });

    if (existingRow) {
      // Update existing row if order amount has changed
      if (existingRow.amounta !== parseFloat($(this).find('.entrySellText[data-book-row-amounta]'))) {
        $(this).find('.entryText').text((existingRow.amounta / 1e8).toFixed(4));
        $(this).find('.entrySellText').text((existingRow.price / 1e8).toFixed(8)).attr('data-book-row-amounta', existingRow.amounta).attr('data-book-row-amountb', existingRow.amountb);
        $(this).find('.marketSizeBar').css('width', ((existingRow.amounta / sellOrdersTotal) * 100) + '%');
      }
    } else {
      // Remove row if it's not present in the server data
      $(this).remove();
    }
  });

  // Add new order data to the order book
  sellEntryContainer.find('.entryContainer').each(function() {
    var existingPrice = parseFloat($(this).find('.entrySellText').attr('data-book-row-price'));

    var newOrder = tmp.find(function(order) {
  ...
  for (var i = 0; i < tmp.length; i++) {
    var order = tmp[i];
    var existingRow = sellEntryContainer.find('.entryText[data-book-row-price="' + order.price + '"]').closest('.entryContainer');
    ...
    if (existingRow.length === 0) {
      ...
      var marketSizeBar = document.createElement('div');
      marketSizeBar.className = 'marketSizeBar marketSizeBarSell';
      marketSizeBar.style.width = '0%';
      entryContainer.appendChild(marketSizeBar);
      ...
      centerLabelCol.className = 'centerLabelCol';
      var centerLabel = document.createElement('span');
      centerLabel.className = 'entrySellText text-red';
      centerLabel.setAttribute('data-book-row-price', order.price);
      centerLabel.setAttribute('data-book-row-amounta', order.amounta);
      centerLabel.setAttribute('data-book-row-amountb', order.amountb);
      centerLabel.textContent = (order.price / 1e8).toFixed(8);
      ...
      sellEntryContainer.append(entryContainer);
