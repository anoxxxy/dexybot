 ...
 * @param {string} options.bidTrend - Trend for bid price ('neutral', 'above', 'below', 'strong_above', 'strong_below')
 * @param {string} options.askTrend - Trend for ask price ('neutral', 'above', 'below', 'strong_above', 'strong_below')
 * @param {number} options.trendAdjustment - Adjustment percentage for trends (default: 0.2%)
 * @param {number|null} options.decimals - Custom decimal precision
 * @returns {Object} Bid and ask prices
 */
function calculateBidAskPrice2(midPrice, options = {}) {
  const {
  ...
  const halfSpreadMultiplier = spreadPercentage / 200;

  // Base bid and ask without trend adjustment
  let bidPrice = midPrice * (1 - halfSpreadMultiplier);
  let askPrice = midPrice * (1 + halfSpreadMultiplier);
  ...
  return {
    ...
    spread: formatPriceForMarket(askPrice - bidPrice, midPrice, decimals),
    spreadPercentage: formatPriceForMarket((askPrice - bidPrice) / midPrice * 100, midPrice, 2)
  };
}


/**
 * Calculate bid and ask prices for market making with dynamic spread adjustment
 * 
 * @param {number} midPrice - Mid-market price
 * @param {Object} options - Configuration options
 * @param {number} options.spreadPercentage - Bid-ask spread as percentage (default: 0.5%)
 * @param {string} options.marketTrend - Overall market trend ('neutral', 'bullish', 'bearish', 'strong_bullish', 'strong_bearish')
 * @param {string} options.bidTrend - Specific trend for bid price ('neutral', 'above', 'below', 'strong_above', 'strong_below')
 * @param {string} options.askTrend - Specific trend for ask price ('neutral', 'above', 'below', 'strong_above', 'strong_below')
 ...
 */
function calculateBidAskPrice1(midPrice, options = {}) {
  const {
    spreadPercentage = 0.5,
    marketTrend = 'neutral',
    bidTrend = 'neutral',
  ...
  switch (marketTrend) {
    case 'bullish':
      // In bullish markets, move bid closer to mid (narrower bid-side spread)
      bidSpreadMultiplier = 1 - (biasAmount * 0.5);
      askSpreadMultiplier = 1 + (biasAmount * 0.5);
      break;
    case 'bearish':
      // In bearish markets, move ask closer to mid (narrower ask-side spread)
      bidSpreadMultiplier = 1 + (biasAmount * 0.5);
      askSpreadMultiplier = 1 - (biasAmount * 0.5);
      break;
    case 'strong_bullish':
      // In strongly bullish markets, move bid much closer to mid
      bidSpreadMultiplier = 1 - biasAmount;
      askSpreadMultiplier = 1 + biasAmount;
      break;
    case 'strong_bearish':
      // In strongly bearish markets, move ask much closer to mid
      bidSpreadMultiplier = 1 + biasAmount;
      askSpreadMultiplier = 1 - biasAmount;
      break;
    case 'neutral':
...
/**
 * Generate a complete market making rule set
 * 
 * @param {number} midPrice - Mid-market price
 * @param {Object} options - Configuration options
...
function generateMarketMakingRules(midPrice, options = {}) {
  ...
  const bidTickPrices = [];
  if (bidTicks > 0) {
    const minBidPush = bidPrice * (1 - minPushPercentage / 100);
    const maxBidPush = bidPrice * (1 - maxPushPercentage / 100);

    const ticks = generateTickPrices(maxBidPush, minBidPush, bidTicks, midPrice, decimals);
  ...
  const askTickPrices = [];
  if (askTicks > 0) {
    const minAskPush = askPrice * (1 + minPushPercentage / 100);
    const maxAskPush = askPrice * (1 + maxPushPercentage / 100);

    const ticks = generateTickPrices(minAskPush, maxAskPush, askTicks, midPrice, decimals);
