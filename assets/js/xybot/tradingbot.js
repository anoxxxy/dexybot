...
class tradingbot {
  constructor() {
    ...
    //xy not implemented yet
    //this.loadFromLocalStorage();
    
  }

  // Method to configure the bot properties
  config({botId, tradingPair, buyBalance = 0, sellBalance = 0, minimalSpread, minOrderCost = 0.00002500, maxOrderCost = 0.00005000, maxOpenOrders = {bid: 3, ask: 3}, intervalPeriod = [], useLiquidity = false, waitTimeRange = {min: 10, max: 20}, cancelOrdersOnStop = { cancelBuyOrders: true, cancelSellOrders: true }, orderType = 'both', bidSettings, askSettings}) {
    ...
    this.maxOpenBidOrders = Math.max(maxOpenOrders.bid, 1); // Enforce minimum value of 1
    this.maxOpenAskOrders = Math.max(maxOpenOrders.ask, 1); // Enforce minimum value of 1
    this.openBidOrders = [];
    this.openAskOrders = [];
  ...
  getMarketData = () => {
    ...
    /*if (['above', 'below'].includes(this.bidSettings.action.trend)) {
      // Ensure that minimum prices are at least 1, and adjust maximum prices if lower than or equal to the minimum by setting them to minimum price plus 1.
      this.bidSettings.action.minPrice = this.bidSettings.action.minPrice < 1 ? 1 : this.bidSettings.action.minPrice;
      this.bidSettings.action.maxPrice = this.bidSettings.action.maxPrice <= this.bidSettings.action.minPrice ? this.bidSettings.action.minPrice + 1 : this.bidSettings.action.maxPrice;

      // Ensure minimum quantity (minQty) aligns with the minimum order cost (minOrderCost/minOrderQty)
      // if minQty is lower. Adjust minQty accordingly.
      if (this.bidSettings.action.minQty < this.tradingPairData.bid.minOrderCost) {
        //this.bidSettings.action.minQty = this.bidSettings.action.minQty < this.tradingPairData.bid.minOrderCost ? this.tradingPairData.bid.minOrderCost : this.bidSettings.action.minQty;
        this.bidSettings.action.minQty = this.tradingPairData.bid.minOrderCost;
      }
      
      // Ensure that the maximum quantity (maxQty) aligns with the minimum quantity (minQty)
      // if maxQty is lower than minQty, set it to minQty
      if (this.bidSettings.action.maxQty < this.tradingPairData.bid.minQty) {
        this.bidSettings.action.maxQty = this.bidSettings.action.minQty;
      }

    }

    if (['above', 'below'].includes(this.askSettings.action.trend)) {
      // Ensure that minimum prices are at least 1, and adjust maximum prices if lower than or equal to the minimum by setting them to minimum price plus 1.
      this.askSettings.action.minPrice = this.askSettings.action.minPrice < 1 ? 1 : this.askSettings.action.minPrice;
      this.askSettings.action.maxPrice = this.askSettings.action.maxPrice <= this.askSettings.action.minPrice ? this.askSettings.action.minPrice + 1 : this.askSettings.action.maxPrice;

      // Ensure minimum quantity (minQty) aligns with the minimum order cost (minOrderCost/minOrderQty)
      // if minQty is lower. Adjust minQty accordingly.
      if (this.askSettings.action.minQty < this.tradingPairData.bid.minOrderCost) {
        //this.askSettings.action.minQty = this.askSettings.action.minQty < this.tradingPairData.bid.minOrderCost ? this.tradingPairData.bid.minOrderCost : this.askSettings.action.minQty;
        this.askSettings.action.minQty = this.tradingPairData.bid.minOrderCost;
      }
      
      // Ensure that the maximum quantity (maxQty) aligns with the minimum quantity (minQty)
      // if maxQty is lower than minQty, set it to minQty
      if (this.askSettings.action.maxQty < this.tradingPairData.bid.minQty) {
        this.askSettings.action.maxQty = this.askSettings.action.minQty;
      }
    ...
    */
    ...
    /*
    // Ensure minimum quantity (minQty) aligns with the minimum order cost (minOrderCost/minOrderQty) if minQty is lower. Adjust minQty accordingly.
    if (this.bidSettings.action.minQty < this.tradingPairData.bid.minOrderCost)
    this.bidSettings.action.minQty = this.bidSettings.action.minQty < this.tradingPairData.bid.minOrderCost ? this.tradingPairData.bid.minOrderCost : this.bidSettings.action.minQty;
    this.askSettings.action.minQty = this.askSettings.action.minQty < this.tradingPairData.ask.minOrderCost ? this.tradingPairData.ask.minOrderCost : this.askSettings.action.minQty;
    */
...
calculateBidOrder_backup = (bidPrice) => {
  const botMinOrderCost = 0.00002500;
  const botMaxOrderCost = 0.00005000;

  let generatedBidPrice = parseFloat(price.toFixed(8));
  //bidPrice = this.askPrice ? this.askPrice : 0.00000010;

  // Generate a random cost within the desired range
  const randomCost = parseFloat((Math.random() * (botMaxOrderCost - botMinOrderCost) + botMinOrderCost).toFixed(8));

  // Calculate the corresponding amount based on the generated bidPrice and the random cost
  const amount = parseFloat((randomCost / generatedBidPrice).toFixed(8));

  return [generatedBidPrice, amount, randomCost];
};
...
handleEmptyBalance = async (orderType, amount) => {
  ...
  if (balance < amount) {
    ...
    console.log('handleEmptyBalance if (balance < amount) : '+ balance + ' < '+ amount);
...
placeAskOrder = async (price, amount, cost) => {
  ...
  fetchBidAndAskPrices = () => {
    ...
    /*let askPriceLowest = ych.data.sells[this.tradingPair]?.[0]?.price || 0;
    let askOrders = ych.data.sells[this.tradingPair];
    // Get the highest ask price from the askOrders array using optional chaining.
    // If the array is empty or the price of the last element is null/undefined, set askPriceHighest to 0.
    let askPriceHighest = askOrders?.[askOrders.length - 1]?.price ?? 0;

    askPriceLowest = Number(askPriceLowest);
    askPriceHighest = Number(askPriceHighest);

    bidPrice = (Number(bidPrice) / 1e8);
    let askPrice = (Math.min(askPriceLowest, askPriceHighest) / 1e8);
    */
...
// Retrieve the buy and sell balance
  ...
  async loadFromLocalStorage() {
    ...
    if (botData) {
      ...
      this.intervalPeriod = this.intervalPeriod.map(Number);
    }
  }
}
